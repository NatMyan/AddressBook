...
AddressBookLogic::AddressBookLogic(QObject *parent) : 
    ...
    db(new Database("../zinfo/users.db"))
{}
...
QString AddressBookLogic::getUsername() {
    return username_;
}
...
void AddressBookLogic::addContact() {
    ...
    if (result == QDialog::Accepted) {
        ...
        query.prepare("INSERT INTO " + username_ + " (name, phone, email, belonging) VALUES (:name, :phone, :email, :belonging)");
...
void AddressBookLogic::searchContacts() {
    ...
    if (searchDialog.exec()) {
        ...
        QSqlQuery query;
        QString queryString = "SELECT * FROM " + username_ + " WHERE " + searchOption + " LIKE '%" + searchName + "%'";
...
void AddressBookLogic::setUsername(QString& username) {
    username_ = username;
}
...
void AddressBookLogic::signIn(QString &username, QString &password) {
    ...
    if (checkCredentials(username, hashedPassword)) {
        ...
        // std::swap(db, newDB);
        db->openDatabase("../zinfo/users.db", username, "(name TEXT, phone TEXT, email TEXT, belonging TEXT)");
...
QByteArray AddressBookLogic::hashPassword(const QString &password) {
