...
void Database::openDatabase(const QString &filePath, const QString &name, const QString &fields) {
    if (!filePath.isEmpty()) {
...
bool Database::createTable(const QString &fileName) {
    QSqlQuery query(db_);
    
    query.exec("CREATE TABLE IF NOT EXISTS" + fileName + " (name TEXT, phone TEXT, email TEXT, belonging TEXT)");
...
QList<Contact> Database::readContacts(const QString &name) {
    QList<Contact> contacts;

    QSqlQuery query(db_);
    if (query.exec("SELECT name, phone, email, belonging FROM " + name)) {
        while (query.next()) {
            Contact contact;
...
bool Database::writeContacts(const QList<Contact> &contacts, const QString &name) {
    QSqlQuery query(db_);

    if (!query.exec("DELETE FROM " + name)) {
        qDebug() << "Error clearing contacts:" << query.lastError().text();
        return false;
    }

    for (const auto &contact : contacts) {
        query.prepare("INSERT INTO " + name + " (name, phone, email, belonging) VALUES (?, ?, ?, ?)");
